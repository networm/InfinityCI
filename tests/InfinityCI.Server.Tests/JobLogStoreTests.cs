...
public class JobLogStoreTests : IDisposable
{
    ...
    {
        var line0 = await _store.AppendAsync(1, "demo", 1, "build", 0, "first");
        var line1 = await _store.AppendAsync(1, "demo", 1, "build", 1, "second");
        ...
        var raw = await File.ReadAllLinesAsync(_store.LogPath(1, "demo", 1, "build"));
    ...
    {
        await _store.AppendAsync(1, "demo", 1, "build", 0, "from-build");
        await _store.AppendAsync(1, "demo", 1, "test", 0, "from-test");

        var buildLines = await _store.ReadAfterAsync(1, "demo", 1, "build", 0);
        var testLines = await _store.ReadAfterAsync(1, "demo", 1, "test", 0);
    ...
    {
        for (var i = 0; i < 5; i++)
            await _store.AppendAsync(1, "demo", 1, "build", 0, $"line-{i}");

        var all = await _store.ReadAfterAsync(1, "demo", 1, "build", 0);
        var rest = await _store.ReadAfterAsync(1, "demo", 1, "build", 3);
        var none = await _store.ReadAfterAsync(1, "demo", 1, "build", 5);
    ...
    {
        Directory.CreateDirectory(Path.Combine(_dir, "demo", "logs", "1"));
        await File.WriteAllTextAsync(_store.LogPath(1, "demo", 1, "build"),
            """{"line":0,"timestampUtc":"2026-01-01T00:00:00.000Z","stepIndex":0,"text":"ok"}""" + "\n" + """{"line":1,"timestampUtc":"torn""");

        var lines = await _store.ReadAfterAsync(1, "demo", 1, "build", 0);
        var line = Assert.Single(lines);
        Assert.Equal("ok", line.Text);
    }

    [Fact]
    public async Task TaskFirstLayout_LogsLiveUnderWorkflowDirectory()
    {
        await _store.AppendAsync(7, "my-task", 2, "build", 0, "x");

        var expected = Path.Combine(_dir, "my-task", "logs", "2", "build.jsonl");
